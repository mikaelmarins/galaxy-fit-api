...
async function getUserTemplates(userId) {
    ...
    try {
       ...
       ORDER BY t.IS_ACTIVE DESC, t.UPDATED_AT DESC`, { userId }, { outFormat: 4002 } // OUT_FORMAT_OBJECT
        );
...
async function getActiveTemplate(userId) {
    ...
    try {
        ...
        const templateResult = await connection.execute(`SELECT * FROM WORKOUT_TEMPLATES
       WHERE USER_ID = :userId AND IS_ACTIVE = 1`, { userId }, { outFormat: 4002 });
...
async function getTemplateById(id, userId) {
    ...
    try {
        ...
        const templateResult = await connection.execute(`SELECT * FROM WORKOUT_TEMPLATES
       WHERE ID = :id AND USER_ID = :userId`, { id, userId }, { outFormat: 4002 });
        const templates = templateResult.rows;
        if (!templates || templates.length === 0) {
            return null;
        }
 ...
 */
async function getTemplateWithDetailsInternal(connection, template) {
    const daysResult = await connection.execute(`SELECT ID, TITLE, DAY_ORDER, DAY_OF_WEEK, CARDIO_DATA
     FROM WORKOUT_DAYS
     WHERE TEMPLATE_ID = :templateId
     ORDER BY DAY_ORDER`, { templateId: template.ID }, { outFormat: 4002 });
    const days = [];
    for (const day of (daysResult.rows || [])) {
        const exercisesResult = await connection.execute(`SELECT ID, NAME, DESCRIPTION, NOTES, REST_SECONDS, EXERCISE_ORDER, IS_STANDARD_SETS, SETS_DATA, EXERCISE_TYPE, CARDIO_DATA
       FROM WORKOUT_EXERCISES
       WHERE DAY_ID = :dayId
       ORDER BY EXERCISE_ORDER`, { dayId: day.ID }, { outFormat: 4002 });
...
async function createTemplate(userId, input) {
    ...
    try {
        ...
        try {
            await connection.execute(`INSERT INTO WORKOUT_TEMPLATES (ID, USER_ID, TITLE, DESCRIPTION, IS_ACTIVE, CREATED_AT, UPDATED_AT, RECOMMENDED_WEEKS)
       VALUES (:id, :userId, :title, :description, 0, :createdAt, :updatedAt, :recommendedWeeks)`, { ...tplData, recommendedWeeks: input.recommendedWeeks || 8 }, { autoCommit: false });
        }
        catch (colErr) {
            ...
            if (colErr.message?.includes('RECOMMENDED_WEEKS')) {
                ...
                await connection.execute(`INSERT INTO WORKOUT_TEMPLATES (ID, USER_ID, TITLE, DESCRIPTION, IS_ACTIVE, CREATED_AT, UPDATED_AT)
       VALUES (:id, :userId, :title, :description, 0, :createdAt, :updatedAt)`, tplData, { autoCommit: false });
            }
            else {
                throw colErr;
            }
        ...
        for (let i = 0; i < input.days.length; i++) {
            ...
            await connection.execute(`INSERT INTO WORKOUT_DAYS (ID, TEMPLATE_ID, TITLE, DAY_ORDER, DAY_OF_WEEK, CARDIO_DATA)
         VALUES (:id, :templateId, :title, :dayOrder, :dayOfWeek, :cardioData)`, dayData, { autoCommit: false });
            for (let j = 0; j < day.exercises.length; j++) {
                const ex = day.exercises[j];
                const exId = (0, uuid_1.v4)();
                const exData = {
                    id: exId,
                    dayId: dayId,
                ...
                await connection.execute(`INSERT INTO WORKOUT_EXERCISES 
           (ID, DAY_ID, NAME, DESCRIPTION, NOTES, REST_SECONDS, EXERCISE_ORDER, IS_STANDARD_SETS, SETS_DATA, EXERCISE_TYPE, CARDIO_DATA)
           VALUES (:id, :dayId, :name, :description, :notes, :restSeconds, :exerciseOrder, :isStandardSets, :setsData, :exerciseType, :cardioData)`, exData, { autoCommit: false });
            }
        }
        // Commit all changes
 ...
 */
async function updateTemplate(id, userId, input) {
    // Verificar se existe
    ...
    try {
        const now = new Date();
        // Atualizar template
        await connection.execute(`UPDATE WORKOUT_TEMPLATES 
       ...
       WHERE ID = :id AND USER_ID = :userId`, {
        ...
        await connection.execute(`DELETE FROM WORKOUT_DAYS WHERE TEMPLATE_ID = :templateId`, { templateId: id }, { autoCommit: false });
        // Inserir novos dias
        for (let i = 0; i < input.days.length; i++) {
            const day = input.days[i];
            const dayId = (0, uuid_1.v4)();
 ...
 */
async function activateTemplate(id, userId) {
    ...
    try {
        ...
        let result;
        try {
            result = await connection.execute(`UPDATE WORKOUT_TEMPLATES 
                 ...
                 WHERE USER_ID = :userId`, { id, userId, now: new Date() }, { autoCommit: true });
        }
        catch (colErr) {
        ...
        console.log('[Templates] Rows affected:', result.rowsAffected);
        // Verificar se o template específico foi atualizado
        const checkResult = await connection.execute(`SELECT IS_ACTIVE FROM WORKOUT_TEMPLATES WHERE ID = :id AND USER_ID = :userId`, { id, userId }, { outFormat: 4002 });
...
async function deleteTemplate(id, userId) {
    const connection = await (0, database_1.getConnection)();
    try {
        const result = await connection.execute(`DELETE FROM WORKOUT_TEMPLATES WHERE ID = :id AND USER_ID = :userId`, { id, userId }, { autoCommit: true });
