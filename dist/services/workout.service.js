...
async function getWorkoutsByUserId(userId) {
    ...
    try {
        const result = await connection.execute(`SELECT ID, USER_ID, WORKOUT_ID, WORKOUT_NAME, START_TIME, END_TIME, 
              DURATION_SECONDS, EXERCISES, USER_WEIGHT, CREATED_AT 
       FROM ${constants_1.TABLES.WORKOUT_LOGS} 
       WHERE USER_ID = :userId 
       ORDER BY END_TIME DESC`, { userId }, { outFormat: database_1.oracledb.OUT_FORMAT_OBJECT });
        if (!result.rows)
            return [];
...
async function getWorkoutById(id, userId) {
    ...
    try {
        const result = await connection.execute(`SELECT ID, USER_ID, WORKOUT_ID, WORKOUT_NAME, START_TIME, END_TIME, 
              DURATION_SECONDS, EXERCISES, USER_WEIGHT, CREATED_AT 
       FROM ${constants_1.TABLES.WORKOUT_LOGS} 
       WHERE ID = :id AND USER_ID = :userId`, { id, userId }, { outFormat: database_1.oracledb.OUT_FORMAT_OBJECT });
...
async function createWorkout(userId, data) {
    ...
    try {
        ...
        await connection.execute(`INSERT INTO ${constants_1.TABLES.WORKOUT_LOGS} 
...
async function updateWorkout(id, userId, data) {
    ...
    try {
        ...
        await connection.execute(`UPDATE ${constants_1.TABLES.WORKOUT_LOGS} SET ${updates.join(', ')} WHERE ID = :id AND USER_ID = :userId`, binds, { autoCommit: true });
        return getWorkoutById(id, userId);
    }
    finally {
        await connection.close();
...
async function deleteWorkout(id, userId) {
    const connection = await (0, database_1.getConnection)();
    try {
        const result = await connection.execute(`DELETE FROM ${constants_1.TABLES.WORKOUT_LOGS} WHERE ID = :id AND USER_ID = :userId`, { id, userId }, { autoCommit: true });
...
async function getWorkoutStats(userId) {
    ...
    try {
       ...
       FROM ${constants_1.TABLES.WORKOUT_LOGS} WHERE USER_ID = :userId`, { userId }, { outFormat: database_1.oracledb.OUT_FORMAT_OBJECT });
        const row = result.rows?.[0];
        return {
